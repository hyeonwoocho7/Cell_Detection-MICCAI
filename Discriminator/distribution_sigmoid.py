...
import cv2
from Discriminator.resnet_dropout import *
import argparse
import os
  ...
  """
    ...
    if step > 0:
        parser.add_argument(
            ...
            dest="input_path",
            help="dataset's path",
            default="/home/hyeonwoo/research/Experiment1/Result/Discriminator/step{}/remove_pgt".format(str(step-1)),
            type=str,
        )
    else:
        parser.add_argument(
            ...
            dest="input_path",
            help="dataset's path",
            default="/home/hyeonwoo/research/Experiment1/Result/Detection/step{}/".format(str(step)),
            type=str,
        )
    parser.add_argument(
        "-o",
        ...
        dest="output_path",
        help="output path",
        default="/home/hyeonwoo/research/Experiment1/Result/Discriminator/step{}/sigmoid_distribution".format(str(step)),
        type=str,
    )
    ...
    parser.add_argument(
        ...
        default="/home/hyeonwoo/research/Experiment1/Model/Discriminator/step{}/best.pth".format(str(step)),
    )
    parser.add_argument(
...
class Predict:
    def __init__(self, args, net, Detection_path, step):
        self.net = net
        self.gpu = args.gpu
        self.step = step
    ...
    def pred(self, ori, pred, ori_path):
        ...
        with torch.no_grad():
            ...
            for i in range(10):
                ...
                with self.save_sigmoid.open(mode="a") as f:
                    f.write("%.12f, %s\n" % (output2, ori_path))
                y1.append(output2)
    ...
    def main(self):
        ...
        n = int(0.1 * len(ori_ps))
        new_ori_paths = random.sample(ori_ps, n)
        for path in new_ori_paths:
            ori = cv2.imread(str(path), 0)
            gt = cv2.imread(str(self.select_pgt / Path(os.path.basename(path))), 0)
            cv2.imwrite("/home/hyeonwoo/research/Experiment1/Data/Detection/step{}/val/ori/".format(str(self.step+1)) + os.path.basename(path), ori)
            cv2.imwrite("/home/hyeonwoo/research/Experiment1/Data/Detection/step{}/val/gt/".format(str(self.step+1)) + os.path.basename(path),gt)

        new_new_paths = list(set(ori_ps) - set(new_ori_paths))
        for p in new_new_paths:
            ori = cv2.imread(str(p), 0)
            gt = cv2.imread(str(self.select_pgt / Path(os.path.basename(p))), 0)
            cv2.imwrite("/home/hyeonwoo/research/Experiment1/Data/Detection/step{}/train/ori/".format(str(self.step + 1)) + os.path.basename(p), ori)
            cv2.imwrite("/home/hyeonwoo/research/Experiment1/Data/Detection/step{}/train/gt/".format(str(self.step + 1)) + os.path.basename(p), gt)
...
def Discriminator_predict(step):
    Detection_path = "/home/hyeonwoo/research/Experiment1/Result/Detection/step{}".format(str(step))
    ...
    pred = Predict(args, net, Detection_path, step)
    pred.main()

Discriminator_predict(1)
